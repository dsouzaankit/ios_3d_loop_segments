...
| **`/export_queue.json`** (PUT or POST, Basic auth) | **Pending FIFO** (not Paused): enqueue `{ "mode"?: "append\|prepend\|replace", "startFirst"?: true, "items": [{ "folderPath"?, "displayName", "seekMs"?, "id"? }] }` → **202**. Or manage `{ "action": "remove", "id" }` / `{ "action": "clear" }` → **200**. LAN `/` + `/browse` list the queue with **Clear queue** at the top + per-item **Remove**. `status.json` always includes `pendingExportQueue`. **`startFirst` while busy:** soft-pauses the live run → **Paused** (parked); new items prepend; drain continues. That interrupted clip is **not** auto-resumed when the FIFO empties — Resume manually. **Resolve miss (278+):** item already popped → ack `rejected` → next drains; **no** in-app/LAN/companion user alert (poll ack or `search_debug.txt`). |
| **`/paused_exports.json`** (GET or POST, Basic auth) | **Paused checkpoints** (not Queued). GET lists count + HTML. POST `{ "action": "clear" }` → **200** clears paused rows / parked media (keeps a live export). POST `{ "action": "queue_all" }` → **200** moves all paused → Queued as fresh jobs (no `seekMs`; drops checkpoints / parked; releases Pause hold; live export kept). LAN **Clear paused** / **Move to queued** use these. |
