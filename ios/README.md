...
| **Vanilla download** | Sparse probe fails; toggle on (default) | WebDAV **full file from 0:00** → **`_vanilla_download.<ext>`**; **60s segments** start at export seek (e.g. 10:00); LAN index adds **`#t=`** on the vanilla link for the same seek (playback at seek needs those bytes downloaded) |
