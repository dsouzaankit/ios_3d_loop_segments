...
- **Keep Alive (optional):** Export tab → **Keep Alive** (above **Exports folder**) → **Keep Alive (lock screen)**. Muted **`AVPlayerLooper`** on the first available file: `loop/op_*.mp4`, `_working.mp4`, active root media, newest **`archive/`** MP4, then bundled **`KeepAlive_silence.mp3`** from [anars/blank-audio](https://github.com/anars/blank-audio) (see **`KeepAlive_silence-credits.txt`** in Resources). Optional **`KeepAlive_tone.caf`**. If playback still fails, in-memory tone / `AVAudioEngine`. Lock screen **Play** / **Pause** / **Stop**; export continues until in-app stop. Logs in **`export_latest.txt`**. **LAN :8765** while export runs (locked OK). Not reliable in Low Power Mode.
