...
`_working.mp4` is **sparse**: the file size and MP4 index at EOF make the browser **scrubber show the full movie duration** (you can drag near the end) even when most of the middle is still empty. **Only dense byte spans** play on LAN — not the scrubber position alone. **Serve Exports on:** MP4 head+index on every export. **Seek 0:** background fill to EOF, then extend dense bytes from playback start through the export cursor each minute. **Seek &gt; 0 (e.g. 10:00):** background from the seek byte offset (not from 0:00), dense **first ~2 min at seek** for browser playback, then extend **seek → cursor+2 min** each minute. Export logs and **`http://&lt;phone-ip&gt;:8765/`** show **`LAN playable till … export …, from …`** (max contiguous dense playback from the seek in file timeline; also in `status.json`). LAN only serves dense bytes — not the scrubber alone.
