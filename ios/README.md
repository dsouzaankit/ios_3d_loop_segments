...
- **Keep Alive (optional):** Export tab → **Keep Alive** (above **Exports folder**) → **Keep Alive (lock screen)**. Silent loop via in-memory `AVAudioPlayer` (falls back to `AVAudioEngine`; `audio` background mode; no disk file). Failures log the step in **`export_latest.txt`** (e.g. `setActive`, `AVAudioPlayer init`). Lock screen **Play** resumes/restarts, **Pause** pauses, **Stop** ends the loop (export continues). Optional auto-stop after 1–8 h. Keep Alive lines also appear in **`export_latest.txt`** while export runs. While export runs, **LAN :8765** and **export triggers** keep polling when the phone is locked (WebDAV still `admin` / `iosadmin`). Not reliable in Low Power Mode.
