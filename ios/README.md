...
**IPA / install (no Mac):** from repo root run **`.\deploy.ps1`** (triggers [ios-build](https://github.com/dsouzaankit/ios_3d_loop_segments/actions/workflows/ios-build.yml), downloads the IPA, then runs **`.\copy-to-icloud.ps1`**). Paste-only: **`.\copy-to-icloud.ps1`**. AltServer tray prep is **skipped by default** (`-EnsureAltStorePrep` for AltStore). Prefer **SideStore nightly** (LocalDevVPN on) or **AltStore**; details: [BUILD-WITHOUT-MAC.md](BUILD-WITHOUT-MAC.md). Companion/USB/setup skip AltServer unless **`-EnsureAltServer`**. PC notes: [../windows/README.md](../windows/README.md).
...
**pCloud WebDAV 404 (account/service):** Phone Sign-in and vanilla/export **need** pCloud WebDAV. If **authenticated** PROPFIND/GET returns **HTTP 404** even on a folder that listed yesterday (e.g. `/p_cld_media/`), while **unauthenticated** still gets **401**, the host is up and this is usually a **pCloud WebDAV outage** — same 404 in a browser GET on `webdav.pcloud.com`, Cyberduck, VLC. Not Depth-0, VPN, 2FA, or spaces in filenames. Confirm on the PC first; wait for pCloud (paid WebDAV, US host, login-confirm email). Builds **305–306** still open **Browse** when the REST API accepts the password, but **new downloads/exports stay blocked** until WebDAV lists again. LAN of already-exported files still works. Europe API **1022 Please provide 'code'** is the **wrong datacenter**, not 2FA (306 retries US).
...
**Build 308 (1.2.72):** LAN probe notice/button say **Phone Wi‑Fi → www** (was bare Wi‑Fi → www).

**Build 307 (1.2.71):** LAN **Phone Wi‑Fi → www** probe (`POST /wifi_www_probe.json`, `allowsCellularAccess = false`) + monitor button; `status.json` includes `wifiWwwProbe`. Companion quit runs the probe before USB Home and bounces the phone LAN AP on fail.
...
**Build 301 (1.2.65):** pCloud **Sign in** is **WebDAV-only again** (US+EU in parallel). Build 300 waited on REST `userinfo` (plain+digest retries) before WebDAV, so login could hang even when WebDAV was fine. API token still loads in the background after a successful WebDAV login.
...
| **`/browse`** | Same middle section as monitor when refreshed, plus **Export random in folder**, **Trim media**, **Clear media**, **Export from URL**, pCloud folder browser (JSON APIs, not browser WebDAV). Auto-refresh when export is idle; manual refresh only while export runs. |
...
| **Bottom** (`/browse` only) | **Export random in folder**, **Trim media**, **Clear media**, trigger status — then **↑ Up** / path / **Refresh** / bookmark; bookmarked folders, folder grid, file list, sort by **name / size / date**. Below that: **Export from URL** (HTTP(S) URL + save name → same vanilla → segments pipeline as browse Export). |
...
| **`/export_from_folder.json`** (PUT or POST, Basic auth) | Queue **Export from folder/filename**: `{ "folderPath"?: "/Videos/MyFolder/", "displayName": "clip.mp4", "seekMs"?: 0, "id"?: "…" }`. Returns **202**. With `folderPath`: one-level PROPFIND only — miss → **Unavailable** on Paused, ack `rejected` (no walk, 289+). **Without `folderPath`:** WebDAV filename walk only. `saveName` / `name` alias `displayName`. Used by [`windows/pcloud_web_companion`](../windows/pcloud_web_companion) — when folder resolve fails, POST `{ "saveName": "…" }` (omit folderPath). |
| **`/export_queue.json`** (PUT or POST, Basic auth) | **Pending FIFO** (not Paused): enqueue `{ "mode"?: "append\|prepend\|replace", "startFirst"?: true, "items": [{ "folderPath"?, "displayName", "seekMs"?, "id"? }] }` → **202**. Or manage `{ "action": "remove", "id" }` / `{ "action": "clear" }` → **200**. LAN `/` + `/browse` list the queue with **Clear queue** at the top + per-item **Remove**. `status.json` always includes `pendingExportQueue`. **`startFirst` while busy:** soft-pauses the live run → **Paused** (parked); new items prepend; drain continues. That interrupted clip is **not** auto-resumed when the FIFO empties — Resume manually. Drain waits while the first item is still resolving its folder (294+) so a 2-file batch cannot start #2 early and then park it on Paused. **Resolve miss (289+):** item popped → ack `rejected` → next drains; skip appears on **Paused** as **Unavailable** (not vanished). Companion toast on the **first** batch ack: `Loop Segments: skipped <name>`. |
| **`/paused_exports.json`** (GET or POST, Basic auth) | **Paused checkpoints** (not Queued). GET lists count + HTML. POST `{ "action": "clear" }` → **200** clears paused rows / parked media (keeps a live export). POST `{ "action": "queue_all" }` → **200** moves available paused → Queued as fresh jobs (no `seekMs`; drops checkpoints / parked; releases Pause hold; live export kept; **Unavailable** rows stay on Paused). LAN **Clear paused** / **Move to queued** use these. |
...
| `command` | Behavior |
|-----------|----------|
| **`start_export`** | Export from `seekMs`. Prefer **`folderPath` + `displayName`** (or `saveName`): phone lists that folder once and matches the filename. If that one-level list/match fails, **do not walk** (289+) — mark **Unavailable** on Paused, ack `rejected`. **Without `folderPath`:** filename WebDAV walk (bookmarks + recent folders). Or pass **`href` + `displayName`**. If both folder and `href` are present, **folder resolve wins**. If an export is running: **soft-pause** immediately (media kept), resolve the new target, then **park + start** only on success — a failed resolve leaves the previous export paused. Soft-paused title stays on **Paused** until **Resume** / **`resume_export`** — FIFO drain does **not** auto-resume it. Same handoff as in-app Start when the new job actually begins. |
...
**Ack** (`export_trigger.ack.json`): `{ "receivedAt", "command", "status", "message", "triggerId" }` where **`status`** is `accepted` \| `rejected` \| `ignored`. Phone-side folder resolve misses (single **`/export_from_folder.json`** / `start_export` **or** a FIFO item with `folderPath`) write **`rejected`** and park an **Unavailable** row on **Paused** (289+; no alert/banner). Queue: item already popped → next drains. Single request: nothing starts (if an export was running, soft-pause leaves it paused). Companion toast on the **first** rejected ack: `Loop Segments: skipped <name>`. Later FIFO skips are Paused-only — poll ack or `search_debug.txt`. LAN Export buttons that poll ack can show `rejected` in the page status line.
