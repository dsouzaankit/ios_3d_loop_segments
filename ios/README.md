...
| **`/paused_exports.json`** (GET or POST, Basic auth) | **Paused checkpoints** (not Queued). GET lists count + HTML. POST `{ "action": "clear" }` → **200** clears paused rows / parked media (keeps a live export). LAN **Clear paused** uses this. |
