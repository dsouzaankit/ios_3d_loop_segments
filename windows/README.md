...
| Home on quit | Companion finish backgrounds Loop Segments over USB (`usb\Go-IphoneHomeViaUsb.ps1`) **in-process** (**only if it is still the foreground app**). Lock screen or already-backgrounded → skip (`HOME_USB_EXIT:0`). Prefers **DVT `--userspace`** (SpringBoard, then Settings) — the same path as USB launch. **`core-device hid --userspace`** is skipped by default (iOS 26 RSD `TimeoutError` typer dump); pass `-TryUserspaceHid` to try it anyway. No USB → skip. Each attempt times out (~25s); `-SkipGoHome` never presses Home. A failed Home press does **not** fail the companion session (still exit 0), but the window **waits for Enter** so the dump stays readable. Direct runs of the Home script also wait on error unless `-NoWaitEnter` (throws `HOME_USB_EXIT:<code>` so the companion host is not killed). Finish also stops the phone rclone mount and runs Wi‑Fi→www AP bounce **in-process** so a dead WinFsp `L:` cannot hang a nested `pwsh` on `InitializeDefaultDrives`. Export continues in background only if the app’s **Keep Alive** is on (default since build 272 — details in [../ios/README.md](../ios/README.md)) |
...
| `lan\Invoke-LoopSegmentsGatewayWifiRebootIfNeeded.ps1` | Wrong-subnet **loop** (wait tcp/23 → reboot → wait up to ~20s for new PC LAN IP or AP back on telnet → re-check, max 3 rounds; telnet fail continues) / forced / **off-subnet sequential** / **`-RebootOtherRouters`** (low Mbps: every known AP except current gateway) / **`-BouncePhoneLanAp`** (phone LAN page subnet AP after Wi‑Fi→www probe fail; ~20s tcp/23 wait; if WifiRestart drops the telnet session then tcp/23 returns, treats that as success and does **not** reboot twice; exits 1 if bounce does not confirm). **Off-subnet / other-router passes skip APs with closed tcp/23** (avoids ~60s telnet WinError 10060). Direct run waits for Enter; companion passes `-NoWaitEnter`. Quit-time bounce also passes **`-InProcessThrowExit`**: sets `LoopSegmentsGatewayExitCode` and throws `GATEWAY_EXIT:<code>` (including **0**); the script’s outer `try/catch` swallows that quietly (no red stack). No nested `pwsh` / dead-`L:` hang. |
| `lan\Invoke-LoopSegmentsPhoneLanRecoverIfNeeded.ps1` | In-process USB/`pcapd` via **`env_setup\lan`** (phone IP vs PC subnet), then wait for `:8765`. If already on-subnet, does not reboot just because the app is down. Idle phone (Wi‑Fi up, pcapd no host IP) → **soft exit 4**, no PC-gateway WifiRestart; recover waits on `:8765` (then off-subnet router reboot only if still down). `-NoWaitEnter` throws `LAN_RECOVER_EXIT:<code>` so companion/rclone are not killed. |
