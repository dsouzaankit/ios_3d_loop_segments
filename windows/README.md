...
**Multi-select tip:** in my.pcloud.com, click the **`v`** control to filter the folder by one of **five** types (including **Video**), then multi-select → Download — the companion cancels the zip and queues videos on the phone FIFO. **Folder right-click → Download is not supported** (zip cancelled, no `fileid`s → “no selection ids”); open the folder, select the videos, then Download instead. **CDN view tabs stay open** (Open Original / inline play); the downloads shelf is still cancelled. Details: [`pcloud_web_companion\README.md`](pcloud_web_companion/README.md).
