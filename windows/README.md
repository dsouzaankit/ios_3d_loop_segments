...
| Home on quit | Companion finish presses **Home** over USB (`Go-IphoneHomeViaUsb.ps1`) when safe. Skips if phone LAN reports an **active export** (exit **4**) so export is not paused — use `-ForceGoHome` to override. Also needs USB + unlocked phone; `-SkipGoHome` leaves app foreground. Keep Alive details: [../ios/README.md](../ios/README.md) |
