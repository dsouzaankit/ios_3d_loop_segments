...
| `-SkipWifiWwwProbeOnQuit` | Do not POST `/wifi_www_probe.json` before Home (default: on-device Wi‑Fi-only www check; on fail bounce the phone LAN AP via `-BouncePhoneLanAp`, unless an export is running or the pending queue is non-empty) |
...
- **SKYBOX VR desktop:** after Chromium is up, companion dotsources repo-root **`Skybox_vr_pc`** (`Start-SkyboxVrPcProcess` / `Map-SkyboxVrPcShare` / `Unmap-SkyboxVrPcShare`) — starts SKYBOX if idle, hides to tray, and maps the AirScreen share from that submodule’s `skybox_vr_pc.config.json` (default **`P:\p_cld_media`** as `p_cld_media`). After rclone finalizes a drive letter it also maps phone **`pcld_ios_media`** (does not change `3d_fullsbs_trans`). Hide-to-tray is a short first-show pass (Skybox recreates its window a few times); it **does not keep hiding** after the window has stayed in the tray, so a manual restore from the notify icon sticks. Missing install only warns. `-SkipSkybox` to skip. Companion quit/abort **unmaps both shares while Skybox is still up**, then **quits SKYBOX if this companion (or a prior companion that left a start marker) started it**. A new companion run **keeps** that marker while SKYBOX is still running so a crash/relaunch does not orphan the client.
...
- **USB-launches Loop Segments** via `..\usb\Launch-LoopSegmentsViaUsb.ps1` **after Chromium starts**. **Skips relaunch** when pymobiledevice3 already sees the app in the **foreground** over USB. Otherwise DVT launch uses **`--no-kill-existing`** so a Keep Alive / lock-screen export is only brought forward (not killed). **Locked (exit 3)** no longer aborts Chromium (warns instead). If USB is missing / launch fails, Chromium stays open and the plugin queues until `:8765` is up. Use `-SkipUsbLaunch` for Chromium only. **AltServer is skipped by default** (SideStore path). Pass **`-EnsureAltServer`** to print status and start AltServer when installed but idle (`..\env_setup\altserver_refresh` via `..\lib\Get-LoopSegmentsAltServer.ps1`). If the app becomes unavailable after ~7 days: **SideStore** (LocalDevVPN → Refresh) or **AltServer → USB → AltStore Refresh All** → **Settings → General → VPN & Device Management → Developer App → Trust → open once**.
- **Attempts rclone mount** via `..\rclone\Mount-LoopSegmentsRclone.ps1 -Quick` in a **separate** console after Chromium is up (preferred letter from `loop-segments-windows.json` `mountDriveLetter`, default `L:`). If that letter is taken, a **random free D–Z** is chosen and saved. After the letter appears, that script notifies Explorer so **This PC** shows it without F5; it notifies again when the mount window exits or LAN-watch unmaps it. **`..\lan\Invoke-LoopSegmentsPhoneLanRecoverIfNeeded.ps1`** runs **in-process** (no nested `pwsh` from `P:`) and uses USB/`pcapd` to check the phone vs PC subnet (`env_setup\lan`), then waits for the LAN page; if USB is missing and `:8765` stays down, it **reboots off-subnet routers** (ROUTER_IPs outside `phoneLanHost`’s subnet) and waits again. Failures only warn. Mount polls phone LAN and auto-kills rclone after ~60s outage. Log: `%TEMP%\loopsegments-rclone-mount.log` (copied to `..\rclone\loopsegments-rclone-mount.log` on quit). Use `-SkipRcloneMount` to leave mounting to `Mount-PhoneL.cmd`. Mount window is minimized and does not steal Chromium focus — **Ctrl+C** there (taskbar / Alt-Tab) to unmount. Use `-SkipGatewayReboot` to skip off-subnet recovery too.
- **LAN throughput probe:** after mount attempt, runs `..\lan\Measure-LoopSegmentsLanThroughput.ps1` (random media under `pcld_ios_media\archive\` ≥ min size, default **64 MB** transfer cap). Prefers HTTP + rclone copy when `${mountDriveLetter}:\` is up (`Test-Path`); if the letter is not ready it **warns and runs HTTP-only**. Prints Mbps, recommends a **max media bitrate** for minute segments (80% of LAN), and writes sidecars for `run_batch_vr_hybrid.ps1`. If Mbps is below `minLanThroughputMbps` (default **40**), reboots Wi‑Fi on **other** known routers (not this PC’s app-LAN gateway), waits to settle, and re-measures — up to **2** retries. Chromium stays open. Low Mbps on the right subnet is **suspected Wi‑Fi channel congestion from neighboring routers**. The app LAN gateway is **tethered to a primary router**; those two APs must use the **same Wi‑Fi channel** (see [`../README.md`](../README.md#app-lan-vs-primary-router)). **Do not run the probe during an active Virtual Desktop headset session** — VD streaming uses ~**50 Mbps** of LAN and will understate phone capacity. Use `-SkipLanThroughput` or `-SkipLowThroughputGatewayReboot` to skip.
...
- **Graceful quit:** close the browser, **Ctrl+C**, or console **X** — kills this profile’s Chromium, **unmaps Skybox Add-folders** (AirScreen **`p_cld_media`** and phone **`pcld_ios_media`**) while SKYBOX is still up, then **quits SKYBOX only if this session started it**, uploads the profile **zip** to P:, clears local AppData (`_profile_exit_watchdog.ps1` covers console X), **stops the phone rclone mount in-process** (kill rclone + mount console; no `Remove-PSDrive` / `Test-Path` on a wedged WinFsp letter), then **POSTs `/wifi_www_probe.json`** (on-device Wi‑Fi-only www; cellular blocked) **before** USB Home — on fail, **bounces the phone LAN AP in-process** (`-BouncePhoneLanAp -InProcessThrowExit`, same `/24` as active `phoneLanHost`; ~20s tcp/23 wait; session drop after WifiRestart that recovers is success, not a second reboot; success exits quietly via outer `try/catch` on `GATEWAY_EXIT:<code>` / `LoopSegmentsGatewayExitCode`, not a red `GATEWAY_EXIT:0` stack) **unless** `status.json` shows an export running or a non-empty pending queue; then USB-Homes Loop Segments **in-process** (`HOME_USB_EXIT:<code>`) **only if it is still foreground** (skipped when already backgrounded or lock screen; DVT `--userspace`; HID Home is skipped on iOS 26). Finish avoids nesting a new `pwsh` so a dead `L:` cannot hang on `InitializeDefaultDrives`. Console transcript: `logs\companion-console-*.log`. Use `-SkipWifiWwwProbeOnQuit` / `-SkipGoHome` / `-NoTranscript` to skip those steps.
