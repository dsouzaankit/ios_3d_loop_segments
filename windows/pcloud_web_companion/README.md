...
| `-SkipSkybox` | Do not check/start SKYBOX VR desktop (default: start if installed but idle) |
| `-SkipVirtualDesktop` | Do not quit/restart Virtual Desktop Streamer and Virtual Desktop Service |
...
- **SKYBOX VR desktop:** after Chromium is up, if the Windows SKYBOX player is installed but not running, starts it and **hides the main window to the tray** (Electron notify icon — not a taskbar minimize). Retries ~30s as the window appears. Missing install only warns. `-SkipSkybox` to skip. If this session started Skybox, companion **quits it** on Chromium close / Ctrl+C / console X / fatal cleanup (a Skybox that was already running is left alone).
- **Virtual Desktop:** after Skybox, **quits Virtual Desktop Streamer if it is running**, **restarts Virtual Desktop Service**, then **starts Streamer** again. Missing install only warns. Service restart may need elevation (Streamer launch often starts the service). `-SkipVirtualDesktop` to skip. Companion finish does **not** quit Streamer.
