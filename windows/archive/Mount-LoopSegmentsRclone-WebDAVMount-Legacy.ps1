...
# ARCHIVED: Phone LAN no longer speaks WebDAV — current app serves HTTP only.
# Kept for reference or very old IPA builds. Active script: ..\Mount-LoopSegmentsRclone.ps1 (HTTP -TestOnly).
<#
.SYNOPSIS
  [ARCHIVED] Mount the phone Documents/Exports folder as a Windows drive via rclone WebDAV.
...
.EXAMPLE
  cd archive
  .\Mount-LoopSegmentsRclone-WebDAVMount-Legacy.ps1 -TestOnly   # old builds only
...
$loopWindowsRoot = Split-Path -Parent $PSScriptRoot
. (Join-Path $loopWindowsRoot 'LoopSegments-Windows.ps1')
